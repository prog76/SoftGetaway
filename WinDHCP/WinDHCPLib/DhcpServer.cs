...
{
    public class OnNewDHCPClientEvent : EventArgs
    {
        public String mac;
        public InternetAddress ip;
    ...
    {
        public event EventHandler<OnNewDHCPClientEvent> OnNewClient;
        ...
        public Socket configureSocket()
        {
            Socket sock = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
            sock.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, 1);
            sock.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.Broadcast, 1);
            sock.Bind(new IPEndPoint(this.m_DhcpInterfaceAddress, DhcpPort));
            return sock;
        }
        ...
        public void Stop()
        {
            ...
            Trace.TraceInformation("Dhcp Stopping.");
            try
            {
                this.m_Abort = true;
                if (this.m_DhcpSocket != null)
                {
        ...
        public void clearReservations()
        {
            lock (this.m_LeaseSync)
            {
                foreach (var v in m_Reservations)
                    removeReservation(v.Key, v.Value);
            }
        }

        public IEnumerable<AddressLease> Leases { get { return m_ActiveLeases.Values; } }

        public IPAddress DhcpInterfaceAddress
        {
            get
            {
            ...
            set
            {
                this.m_DhcpInterfaceAddress = value;
            }
        }
    }
}
